...
class DeepmatrixObservation(Observation):
    ...
    # ------------------------------------------------------------------ #
    # Cumulative KPIs (exposed for graders)                                #
    # ------------------------------------------------------------------ #
    cumulative_service_level: float = Field(
        default=0.0,
        description="Running fill-rate: total fulfilled / total demand so far.",
    )
    cumulative_waste_units: int = Field(
        default=0,
        description="Total units wasted/expired across all weeks so far.",
    )
    cumulative_profit: float = Field(
        default=0.0,
        description="Total profit (revenue - all costs) accumulated so far.",
    )
